...
/*   Updated: 2025/03/28 15:17:14 by hguerrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	get_exp_str_len(t_list **token, char *var_value,
		char *dollar, bool double_quotes)
{
	if (double_quotes == true)
		return (ft_strlen(var_value) + strlen_until_expansion((*token)->str,
				dollar));
	else
		return (strlen_until_spaces(var_value)
			+ strlen_until_expansion((*token)->str, dollar));
}

static char	*expand(t_prompt_info prompt_info, t_list **tokens_list,
		t_list **token)
{
	char	*var_value;
	char	*dollar;
	...
	while (1)
	{
		...
		exp_str_len = get_exp_str_len(token, var_value, dollar, double_quotes);
		(*token)->str = join_var((*token)->str, var_value, dollar,
				var_key_len(dollar + 1));
		if (!(*token)->str)
			return (NULL);
...
t_list	*expand_vars(t_prompt_info prompt_info, t_list **tokens_list)
{
	...
	{
		if (((token->previous
					&& check_redirect_type(token->previous->str) != HEREDOC)
				|| !token->previous) && find_expand_dollar(token->str,
				&double_quotes, false))
		{
