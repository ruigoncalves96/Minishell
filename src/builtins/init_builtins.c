...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/24 12:08:30 by hguerrei          #+#    #+#             */
/*   Updated: 2025/03/24 12:37:56 by hguerrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const char	*builtins[] = {"pwd", "echo", "env", "cd", "export", "unset",
		"exit", NULL};
	int			i;
...
static int	handle_specific_builtin(char **args, t_prompt_info *prompt_info,
		t_builtins *builtins, t_token *tokens)
{
		...
		t_builtins *builtins)
{
	if (!tokens || !tokens->token || !*tokens->token
		|| !is_builtin(*tokens->token))
		return (0);
	builtins->exit_code = 0;
	return (handle_specific_builtin(tokens->token, &prompt_info, builtins,
			tokens));
}
...
static char	*create_env_string(const char *key, const char *value)
{
	...
	env_str = ft_calloc(key_len + value_len + 2, 1);
	if (!env_str)
		return (NULL);
